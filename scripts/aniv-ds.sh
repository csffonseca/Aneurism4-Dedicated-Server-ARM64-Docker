...
validate_server() {
    ...
    if [ -z "$BETA" ]; then
        FEXBash '/home/steam/steamcmd/steamcmd.sh +force_install_dir "$INSTALL_DIR" +login "$USERNAME" +app_update "$APP_ID" validate +quit'
    else
        FEXBash '/home/steam/steamcmd/steamcmd.sh +force_install_dir "$INSTALL_DIR" +login "$USERNAME" +app_update "$APP_ID" -beta "$BETA" validate +quit'
    fi
